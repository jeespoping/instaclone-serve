...
async function register(input) {
  const newUser = input;
  newUser.email = newUser.email.toLowerCase(); //convierte a minusculas las ltras;
...
async function login(input) {
  ...
  const passworSucess = await bcryptjs.compare(password, userFound.password);
  if (!passworSucess) throw new Error("Error en el email o contraseña");
...
async function updateUser(input, ctx) {
  ...
  try {
    if (input.currentPassword && input.newPassword) {
      ...
      const passworSucess = await bcryptjs.compare(
        input.currentPassword,
        userFound.password
      );
      if (!passworSucess) throw new Error("Contrasñea incorrecta");
